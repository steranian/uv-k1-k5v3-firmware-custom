...
// Coordinate lookup table for the analog meter needle tip (Original size: radius 40, center (64,60))
...
// Fast, simple pseudo-random number generation using Xorshift algorithm
...
void APP_RunSpiritBox(void)
{
    ...
    uint8_t current_mode = MODE_ASC; // Initial mode: Ascending
    uint8_t current_mem_idx = 0;     // Current memory index
    ...
    while (isRunning) {
        ...
        if (key != prevKey) {
            if (key == KEY_EXIT) {
                ...
                // Switch scan mode via MENU key (ASC -> DSC -> RND -> ASC...)
        ...
        if (sweep_tick >= sweep_speed_ms) {
            ...
            // --- Switch frequency based on the selected mode ---
            if (valid_count > 1) {
                ...
                // When no channels stored in memory (Sweep full VFO range)
                if (current_mode == MODE_RND) {
        ...
        // R (Left aligned) / V (Right aligned) [Line 1]
